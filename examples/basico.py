"""
Exemplo básico de uso do mcp-fiscal-brasil como biblioteca Python.

Demonstra as operações mais comuns: consulta de CNPJ, validação de CPF
e verificação de status do SEFAZ.
...
async def main() -> None:
    async with FiscalBrasil() as fiscal:
        # --- Validação offline (sem chamada de API, instantânea) ---
        print("=== Validação offline ===")
        print(f"CPF 529.982.247-25 válido: {fiscal.validar_cpf('529.982.247-25')}")
        print(f"CPF 111.111.111-11 válido: {fiscal.validar_cpf('111.111.111-11')}")
        print(f"CNPJ 33.000.167/0001-01 válido: {fiscal.validar_cnpj('33.000.167/0001-01')}")
        print(f"CNPJ 00.000.000/0000-00 válido: {fiscal.validar_cnpj('00.000.000/0000-00')}")

        # Validadores também disponíveis como funções avulsas
